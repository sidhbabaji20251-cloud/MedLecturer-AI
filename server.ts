...
// Initialize dynamic fallback presentation builder to guarantee 100% uptime when Gemini daily free quotas are exhausted
function generateFallbackPresentation(topic: string, specialty: string, targetAudience: string, theme: string, slideCount: number) {
  const slides = [];
  ...
  // Slide 2: CBME Competency Outline
  slides.push({
  ...
  // Intermediate pathophysiological & molecular slides
    ...
    if (i === 1) {
      slides.push({
        ...
        bullets: [
          ...
          `*💡 FUNNY MEMORIZATION MNEMONIC*: **Bhaiya Jaldi Karo, Sab Yaad Ho Jayega!**`,
      ...
      slides.push({
        ...
        bullets: [
          ...
          `*💡 FUNNY COFACTOR MNEMONIC*: **Mera Dost Sabse Best Hai!**`,
  ...
  slides.push({
    ...
    bullets: [
      ...
      `*💡 FUNNY DRUG CLASSES MNEMONIC*: **Babu Bhaiya, Mast Karo!**`,
  ...
  // Dynamic padding to guarantee exact slide count matching
  while (slides.length < limit) {
    slides.splice(slides.length - 1, 0, {
      ...
      bullets: [
        ...
        `*💡 MEMORIZE THIS*: **Dost, Sab Yaad Rakhna!** (Friend, remember everything!).`
      ],
      speakerNotes: "This slide provides additional details for comprehensive course coverage."
    });
  }

  // Enforce precise array slice matching slideCount limit
...
async function startServer() {
  ...
  // API Endpoint for generating slides
...
2. FUNNY MEMORIZATION MNEMONICS (PREFERABLY IN HINDI / HINGLISH):
   ...
   - You MUST prefix these mnemonics explicitly in the bullets as: "💡 FUNNY MEMORIZATION MNEMONIC: [Funny Hindi/Hinglish phrase, followed by a clear English description mapping what each word/letter represents]". Make it extremely engaging, relatable, hilarious, or humorous so students laugh and memorize it forever!
...
`;
  ...
  // Serve static files and handle Vite middlewares
  if (isProd) {
